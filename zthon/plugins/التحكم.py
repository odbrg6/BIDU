...
@zedub.zed_cmd(pattern="التحكم (تفعيل|تعطيل)$")
...
@zedub.zed_cmd(pattern="اضف متحكم(?:\s|$)([\s\S]*)")
...
@zedub.zed_cmd(pattern="ازالة متحكم(?:\s|$)([\s\S]*)")
async def _(event):
    replied_user, error_i_a = await get_user_from_event(event)
    if replied_user is None:
        return
...
@zedub.zed_cmd(pattern="المتحكمين$")
async def _(event):
