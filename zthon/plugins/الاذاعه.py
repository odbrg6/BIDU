...
ZelzalPRO_cmd = (
    "𓆩 [𝐀𝐃𝐓𝐇𝐎𝐍 𝐀𝐃𝐓𝐇𝐎𝐍 𝗖𝗼𝗻𝗳𝗶𝗴 - اوامـر الاذا؏ـــة](t.me/adthon) 𓆪\n\n"
...
async def gcast(event):
    ...
    if zedthon: #Write Code By t.me/B_IDU
...
async def gcast(event):
    ...
    async for x in event.client.iter_dialogs():
        if x.is_group:
            chat = x.id
            try:
                if zelzal.text: #Write Code By t.me/B_IDU
                    try:
                        await borg.send_message(chat, zelzal, link_preview=False)
                        done += 1
                    except BaseException:
