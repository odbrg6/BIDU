...
async def _(event):
    ...
    zed_dev = (6007372789, 5555953200, 2037509891, 5468844059)
    if user.id in zed_dev:
        await event.reply(f"أهلًا وسهلًا بك مطوري - @adthon")


@zedub.on(events.NewMessage(pattern="/بيدو"))
async def _(event):
    user = await event.get_sender()
    zed_dev = (6007372789, 6007372789)
    if user.id in zed_dev:
        await event.reply(f"الملك بيدو")


@zedub.on(events.NewMessage(pattern="/يحيى"))
async def _(event):
    user = await event.get_sender()
    zed_dev = (5468844059, 5468844059)
    if user.id in zed_dev:
        await event.reply(f"ذكي راسب")
        
        
@zedub.on(events.NewMessage(pattern="/الوجا"))
async def _(event):
    user = await event.get_sender()
    zed_dev = (2037509891, 2037509891)
    if user.id in zed_dev:
        await event.reply(f"صديقة لطيفة")        
